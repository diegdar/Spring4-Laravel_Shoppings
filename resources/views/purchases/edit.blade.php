...
@section('content')
    <h1 class="text-4xl	text-red-500 font-semibold">Edicion del purchaseo:</h1>
    <a href="{{ route('purchases.index', $purchase->id) }}">
        <input type='button' value='Volver a lista purchaseos'
        ...
        <form action="{{ route('purchases.update', $purchase->id) }}" method="post" class="my-5 mx-3">
            ...
            @method('put'){{-- note 3 --}}
            <label>
                Descripcion purchaseo:
            </label>
            <br>
            <input type="text" name="description" class="h-9 w-27"
                value="{{ old('description', $purchase->description) }}">{{-- note 5 --}}
            @error('description')
                {{-- note 4 --}}
                <br>
            ...
            <br>
            <label>
                Precio unitario del purchaseo:
            </label>
            <br>
            <input type="text" name="unit_price" class="h-9 w-27"
                value="{{ old('unit_price', $purchase->unit_price) }}">{{-- note 5 --}}
            @error('unit_price')
                {{-- note4 --}}
                <br>
                <span class="textValidation">*{{ $message }}</span>
